...
def group_transactions_by_date_category(transactions: List[Dict]) -> dict[datetime, dict[str, Decimal]]:
    ...
    for row in transactions:
        ...
        if category_name:
            transactions_by_date_category[date][category_name] = transactions_by_date_category[date].get(category_name, Decimal(0)) + amount
...
def create_income_expense_bar_chart(data: List[Dict]) -> None:
    ...
    unique_categories = list(set(category for categories in transactions_by_date_category.values() for category in categories))
